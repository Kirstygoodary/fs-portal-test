    ...
    AbstractSystemPause
{
    ...
    function setPoolInitialized() external onlyManager {
        isInitialized = true;
    }
    ...
    function updateInterestRates(uint256 interestRate) external onlyAdmin {
        require(block.timestamp < raceStartDate, "Staking: Race already began");
        require(interestRate > 99, "Staking: interestRate must be >> than 99");
        require(
            interestRate < maximumInterestRateInBasisPoints,
            "Staking: interestRate must be << than maxInterestRate"
        );
        unbalancedInterestRatePerSecond = StakingLib
            .getInterestRatePerSecondUnbalanced(interestRate);
